...
/** يمنع معاينتين في نفس السيرفر في نفس الوقت. */
...
function buildEmbed(state) {
  ...
  const title =
    state.mode === 'set-sound'
      ? 'قص صوت الدخول (set-sound)'
      : 'قص المقطع قبل الإضافة للمكتبة';
  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      '**باختصار — ماذا تفعل؟**\n' +
        '1) **الصف الأول:** يحرّك **بداية** المقطع في الملف (من أي ثانية يبدأ الاستماع).\n' +
        '2) **الصف الثاني:** يغيّر **كم ثانية** يُحفظ من تلك البداية (حتى **' +
        audioFrom.MAX_AUDIO_SECONDS +
        '** ث).\n' +
        '3) اضغط **▶ استمع** وأنت داخل **روم صوتي** لتسمع النتيجة قبل الحفظ.\n' +
        '4) إذا عجبك الصوت اضغط **حفظ**؛ وإلا عدّل بالأزرار واستمع ثانية.\n\n' +
        `**طول الملف كامل:** ~${total.toFixed(1)} ث\n` +
        `**ما اخترته:** من **${fmtTime(state.startSec)}** إلى **${fmtTime(end)}** — **${state.durationSec.toFixed(1)}** ث\n\n` +
        `\`${bar}\` _موضع المقطع في الملف (تقريبي)_\n\n` +
        '_بديل: يمكنك كتابة الأرقام مباشرة في الأمر: `start_second` و `duration_second`._'
    )
    .setFooter({ text: 'تنتهي هذه الجلسة بعد 10 دقائق بدون نشاط.' });
}
...
async function playTrimPreview(interaction, state) {
  ...
  if (!channel?.isVoiceBased()) {
    return interaction.reply({
      content:
        '**ادخل روم صوتي** في هذا السيرفر، ثم اضغط **▶ استمع** مرة ثانية.\n' +
  ...
  if (previewGuildBusy.has(guild.id)) {
    return interaction.reply({
      content: 'معاينة أخرى قيد التشغيل في السيرفر. انتظر قليلاً ثم جرّب مجدداً.',
      ephemeral: true
    });
  }
  ...
  try {
    ...
    await interaction.followUp({
      content:
        ...
        'إذا كان الصوت مناسباً اضغط **حفظ في المكتبة**، وإلا غيّر البداية أو الطول.',
      ephemeral: true
    });
    ...
    await interaction
      .followUp({
        content:
          'تعذّر الاتصال بالروم الصوتي أو تشغيل المعاينة. تحقق من صلاحيات البوت.',
        ephemeral: true
      })
      .catch(() => {});
  }
